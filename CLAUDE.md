...
| `app/settings.tsx` | Settings — JSON/CSV export, JSON import |
...
| `app/_layout.tsx` | Root layout — font loading, store init, app splash |
...
`Person` and `ConversationLog` are the two core data shapes. `Person.tags` is always `string[]` (never undefined — migration handled in `load()`).
...
| `PersonForm` | Create/edit person form with photo picker and TagInput |
...
### Fonts
`PlayfairDisplay_700Bold_Italic` from `@expo-google-fonts/playfair-display` is used for the "Recall" header title and splash screen. Font loading is handled in `_layout.tsx` with `SplashScreen.preventAutoHideAsync/hideAsync`.

### Backup (`src/utils/backup.ts`)
- Uses `expo-file-system/legacy` (NOT `expo-file-system` — v19 changed the API)
